...
class RGAT_Layer(nn.Module):
    ...
    def __init__(self, ndim, edim, num_heads=1, feat_drop=0.2):
        ...
        self.affine_q = nn.Linear(self.ndim, dim)
        self.affine_k = nn.Linear(self.ndim, dim, bias=False)
        self.affine_v = nn.Linear(self.ndim, dim, bias=False)
    ...
    def forward(self, x, lgx, graph):
        ...
        # lgx: edge features
        g = graph.to(x.device)
        q = self.affine_q(self.feat_dropout(x))
        k = self.affine_k(self.feat_dropout(x))
        v = self.affine_v(self.feat_dropout(x))
        if lgx.size(-1) == q.size(-1):
            e = lgx.view(-1, self.num_heads, self.d_k)
        else:
            e = lgx.unsqueeze(1).expand(-1, self.num_heads, -1)
        with g.local_scope():
    ...
    def propagate_attention(self, g):
        ...
        # Use copy_e instead of copy_edge
