...
from transformers import T5TokenizerFast, AutoConfig, AutoModelForSeq2SeqLM
...
max_seq_length_inference = config.getint('DEFAULT', 'max_seq_length_inference', fallback=max_seq_length)
...
def main():
    ...
    # tokenize
    tr_enc_in  = tokenizer(train_in,  padding=True, truncation=True, return_tensors="pt")
    tr_enc_out = tokenizer(train_out, padding=True, truncation=True, return_tensors="pt")
    dv_enc_in  = tokenizer(dev_in,    padding=True, truncation=True, return_tensors="pt")
    dv_enc_out = tokenizer(dev_out,   padding=True, truncation=True, return_tensors="pt")
    ts_enc_in  = tokenizer(test_in,   padding=True, truncation=True, return_tensors="pt")
    ...
    test_dl  = DataLoader(CustomDataset(ts_enc_in,  ts_enc_out),batch_size=batch_size_inference, shuffle=False)
