...
def perform_inference(model, dl, phase_name="Evaluation"):
    ...
    for batch in pbar:
        ...
        # Clear forward pass output immediately
        del out
        ...
        preds = tokenizer.batch_decode(gen_ids_cpu, skip_special_tokens=True)
        labels = tokenizer.batch_decode(labels_cpu, skip_special_tokens=True)

        # Clear GPU tensors immediately
        del gen_ids, input_ids, attention_mask, graph_idx
        del gen_ids_cpu, labels_cpu
        del batch  # Clear entire batch
        ...
        del proc, truth_proc, preds, labels
...
# -------------------------------- STEPWISE  EVALUATION ------------------------------------------------------------
    ...
    if do_train:
        ...
        while global_step < num_steps:
            ...
            # Clean up training step memory immediately
            loss_val = loss.detach().cpu().item()
            del out, loss
            del batch  # Clear training batch
            ...
            if should_eval:
                # Clear any lingering gradients before evaluation
                ...
                # Critical: Clear evaluation results to prevent memory accumulation
                del val_res
                
                # Force garbage collection and clear GPU cache after evaluation
                gc.collect()
                torch.cuda.empty_cache()
    ...
    if do_test:
        ...
        # Clear any remaining memory before loading test model
        gc.collect()
        torch.cuda.empty_cache()
